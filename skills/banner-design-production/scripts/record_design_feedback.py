#!/usr/bin/env python3
"""デザイン画像のユーザー目視フィードバックをhuman_feedback.mdへ保存する。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--selected-design", default="", help="採用または主対象のデザインID。例: design_1")
...
def update_summary(run_dir: Path, status: str, selected_design: str, feedback_id: str, created_at: str) -> None:
    summary_path = run_dir / "run_summary.json"
    if not summary_path.exists():
        return
...
def main() -> int:
    ...
    section = "\n".join(
        [
            f"# デザインチェック(目視)・承認フィードバック: {feedback_id}",
            "",
            "## メタ情報",
            "",
            ...
            f"- フェーズ: Phase 2 / デザイン目視フィードバック",
            f"- 承認ステータス: {args.approval_status}",
            f"- 対象/採用デザイン: {args.selected_design or '未指定'}",
            "",
            "## フィードバック本文",
            "",
            feedback,
    ...
    update_summary(run_dir, args.approval_status, args.selected_design, feedback_id, created_at)
    record_phase_event(
        run_dir,
        "phase_2",
        ...
        extra={
            ...
            "selected_design": args.selected_design,
        },
    )
