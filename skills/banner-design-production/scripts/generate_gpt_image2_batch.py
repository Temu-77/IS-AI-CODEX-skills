...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--concurrency", type=int, default=2, help="並列数。初期値は2。")
...
def update_run_summary(output_dir: Path, results: list[dict[str, Any]]) -> None:
    ...
    for result in results:
        ...
        if output in by_file:
            by_file[output].update(record)
        else:
...
def main() -> int:
    ...
    update_run_summary(output_dir, results)
