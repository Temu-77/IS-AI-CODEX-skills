...
def update_run_summary(output_path: Path, concept_id: str) -> None:
    summary_path = output_path.parent / "run_summary.json"
    if not summary_path.exists():
        return
    try:
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
    except Exception:  # noqa: BLE001 - best-effort summary update
        return

    outputs = summary.setdefault("outputs", {})
    generated_images = outputs.setdefault("generated_images", [])
    if not isinstance(generated_images, list):
        generated_images = []
        outputs["generated_images"] = generated_images

    output_text = str(output_path)
    metadata_text = str(output_path.with_suffix(output_path.suffix + ".metadata.json"))
    existing = [item for item in generated_images if isinstance(item, dict) and item.get("file") == output_text]
    record = {
    ...
    if existing:
        ...
        generated_images.append(record)
...
def main() -> int:
    ...
    update_run_summary(output_path, args.concept_id)
