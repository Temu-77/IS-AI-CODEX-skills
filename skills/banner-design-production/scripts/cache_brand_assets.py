...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cache-root", help="キャッシュルート。未指定時は$BANNER_DESIGN_CACHE_DIRまたはcwd/.cache/banner-design-production/brand-assets。")
...
def default_cache_root() -> Path:
    configured = os.environ.get("BANNER_DESIGN_CACHE_DIR") or os.environ.get("BANNER_PROPOSAL_CACHE_DIR")
    if configured:
        return Path(configured).expanduser().resolve()
    return (Path.cwd() / ".cache/banner-design-production/brand-assets").resolve()
